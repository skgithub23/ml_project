...
class classification():
     def __init__(self,path='C:/Users/mailf/Downloads/drive-download-20231109T161859Z-001/Train_Data.csv',clf_opt='lr',no_of_selected_features=None):
        self.path = path
...
# Load the data 
    ...
    # Load the file using Pandas       
        reader=pd.read_csv(self.path+'Train_Data.csv')  
    ...
    # Select all rows except the ones belong to particular class'
        ...
        data=reader.iloc[:, :-1]
        labels=reader['price_range']
        #data=pd.get_dummies(data,columns=['blue','dual_sim','four_g','three_g','touch_screen','wifi'])   # For ONEHOT encoding
        #data= data.loc[:,['battery_power','ram','px_height','px_width']]      # For higly corelated features using corelation matrix
        print(data)
        labels=reader['price_range']
